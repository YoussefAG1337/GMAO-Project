...
const passwordMessage = 'Le mot de passe doit contenir au moins 8 caractères, une majuscule, une minuscule, un chiffre et un caractère spécial (@$!%*?&#+_-.)';
...
export const createUserSchema = z.object({
  ...
  email: z.string(required('L\'email est requis')).email().max(255).transform(val => val.toLowerCase().trim()),
  motDePasse: z.string(required('Le mot de passe est requis')).min(8).regex(passwordRegex, passwordMessage),
