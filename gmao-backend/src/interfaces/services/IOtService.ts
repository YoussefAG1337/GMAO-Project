...
export interface IOtService {
  getOTs(filters: any, pageNum: number, limitNum: number, currentUser: { userId: number, role: Role }): Promise<{
    total: number;
    page: number;
    limit: number;
    totalPages: number;
  ...
  submitRapport(id: number, data: SubmitRapportDTO, currentUser: { userId: number, role: Role }): Promise<RapportIntervention>;
