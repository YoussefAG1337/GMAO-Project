...
export const produitService = {
  ...
  familles: {
    getAll: () =>
      api.get<ApiResponse<{ familles: FamilleProduit[] } | FamilleProduit[]>>('/produits/familles').then((res) => {
        const data = res.data;
        return (data && 'familles' in data && Array.isArray(data.familles)) ? data.familles : (data as FamilleProduit[]);
      }),
    ...
    update: (id: number, data: UpdateFamilleDto) =>
      api.put<ApiResponse<FamilleProduit>>(`/produits/familles/${id}`, data).then((res) => res.data),
    delete: (id: number) =>
  ...
  produits: {
    getAll: () =>
      api.get<ApiResponse<{ produits: Produit[] } | Produit[]>>('/produits').then((res) => {
        const data = res.data;
        return (data && 'produits' in data && Array.isArray(data.produits)) ? data.produits : (data as Produit[]);
      }),
